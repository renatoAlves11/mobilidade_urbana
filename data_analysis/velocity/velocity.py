...
# === Filtragem: remover linhas com poucos registros ===
n_min = 1000  # mínimo de registros para considerar a linha
linhas_validas = dados['LINE'].value_counts()[dados['LINE'].value_counts() >= n_min].index

dados_filtrados = dados[dados['LINE'].isin(linhas_validas)]
dados1_filtrados = dados1[dados1['LINE'].isin(linhas_validas)]

#--------------------------------------
# Velocidade média por hora do dia
vel_por_hora = dados_filtrados.groupby(["LINE", "HORA"])["VELOCITY"].mean().reset_index()
...
sns.boxplot(data=dados_filtrados, x="LINE", y="VELOCITY", hue='LINE', palette=colors)
...
plt.figure(figsize=(10,6))
sns.histplot(data=dados1_filtrados, x="VELOCITY", hue="LINE", kde=True, element="step", common_norm=False, palette=colors)
